...
resource "aws_route_table_association" "wp_pulic1_assoc" {
...
resource "aws_security_group" "wp_private_sg" {
  name = "wp_private_sg" 
  description = "Used for access to everything from withing the vpc on the private side"
  vpc_id = "${aws_vpc.wp_vpc.id}"

  # access from internal VPC only
...
resource "aws_elb" "wp_elb" {
  ...
  # LB will distribute over instances within their respective AZ
